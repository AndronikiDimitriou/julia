...
function model(z)
    ...
    # std_of_chunks=view(std(c, dims=2), :,1) #std over chunks

    weight_norm=zeros(length(logbins))
    @threads for i in eachindex(logbins)
        res=sum((0.67^2*Omega_noise.(f_filtered[idx.==i],2.5*1e9,3*1e8,P,A)).^(-1))
    ...
    @threads for i in eachindex(logbins)
        ...
        num = (0.67^2*Omega_noise.(f_filtered[idx.==i],2.5*1e9,3*1e8,P,A)).^(-1)
        w = num./weight_norm[i]
